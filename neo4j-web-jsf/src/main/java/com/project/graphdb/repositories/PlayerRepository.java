...
public class PlayerRepository {
    ...
    private final GraphDatabaseService engine;

    public PlayerRepository(GraphDatabaseService engine) {
        this.engine = engine;
    }
    ...
    public boolean removeTeam(long id, GraphDatabaseService graphDb) {
        ...
        try (Transaction tx = graphDb.beginTx()) {
            ...
            Result result = engine.execute(remove,
                    params);
            tx.success();
        }
        return true;
...
//            }
    ...
    public void updatePlayer(Player entity, GraphDatabaseService graphDb, long teamId) {
        ...
        try (Transaction tx = graphDb.beginTx()) {
            ...
            Result removeRelation = engine.execute(delRel,
                    params);
            //again do rel
    ...
    public List<Player> findAllPlayers() {
        Result result = engine.execute("MATCH (e:Player) RETURN e");
    ...
    public List<Player> findAllPlayersWithTeam() {
        Result result = engine.execute("MATCH (a)-[:`PLAY_IN`]->(b) RETURN ID(a), a.name, a.surname, a.position, ID(b), b.name");
    ...
    public Player findPlayerById(long id) {
        ...
        Result result = engine.execute(query2,
                params);
    ...
    public List<Team> findAllTeams() {
        Result result = engine.execute("MATCH (e:Team) RETURN e");
        Iterator<Node> nodes = result.columnAs("e");
        return fromTeamNodes(nodes);
    }

    public LinkedList<Player> playIn() {
        Result result = engine.execute("MATCH (n) -[:PLAY_IN]->( :Team ) RETURN n");
    ...
    public LinkedList<Player> bySurname(String surname) {
        ...
        Result result = engine.execute("MATCH (n:Player {surname: {surname}}) "
                + "RETURN n",
                params);
        Iterator<Node> nodes = result.columnAs("n");
        return fromNodes(nodes);
    ...
    public LinkedList<Team> findTeamByName(String name) {
        ...
        Result result = engine.execute("MATCH (n:Team {name: {name}}) "
                + "RETURN n",
                params);
        Iterator<Node> nodes = result.columnAs("n");
        return fromTeamNodes(nodes);
